...
class ChartDesign10Activity : AppCompatActivity() {
        ...
        return expenses.filter { expense ->
            ...
            when (period) {
                ...
                "WEEK" -> {
                    ...
                    val weekExpense = calendar.get(Calendar.WEEK_OF_YEAR)
                    weekNow == weekExpense && calendar.get(Calendar.YEAR) == Calendar.getInstance().get(Calendar.YEAR)
                }
                "MONTH" -> {
                    calendar.timeInMillis = now
                    ...
                    calendar.timeInMillis = ts
                    monthNow == calendar.get(Calendar.MONTH) && yearNow == calendar.get(Calendar.YEAR)
                }
                "YEAR" -> {
                    calendar.timeInMillis = now
                    val yearNow = calendar.get(Calendar.YEAR)
    ...
    private fun setupPieChart(pieChart: PieChart, categoryTotals: Map<String, Float>) {
        ...
        categoryTotals.forEach { (category, total) ->
            if (total > 0f) entries.add(PieEntry(total, category))
        }
        ...
        pieChart.apply {
            ...
            centerText = ""                // remove center text
            description.isEnabled = false
            setDrawEntryLabels(false)      // remove slice labels

            legend.apply {
                orientation = Legend.LegendOrientation.HORIZONTAL
                horizontalAlignment = Legend.LegendHorizontalAlignment.CENTER
                verticalAlignment = Legend.LegendVerticalAlignment.BOTTOM
                isWordWrapEnabled = true   // wrap legend to multiple lines
                textSize = 12f
